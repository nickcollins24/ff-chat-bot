package ncollins.model.chat;

public final class Emojis {
    public static final String BEER_MUG =       "\uD83C\uDF7A"; //🍺
    public static final String FOOTBALL =       "\uD83C\uDFC8"; //🏈
    public static final String EYES_LEFT =      "\uD83D\uDC40"; //👀
    public static final String EYE =            "\uD83D\uDC41"; //👁
    public static final String EAR =            "\uD83D\uDC42"; //👂
    public static final String NOSE =           "\uD83D\uDC43"; //👃
    public static final String LIPS =           "\uD83D\uDC44"; //👄
    public static final String TONGUE =         "\uD83D\uDC45"; //👅
    public static final String FINGER_UP =      "\uD83D\uDC46"; //👆
    public static final String FINGER_DOWN =    "\uD83D\uDC47"; //👇
    public static final String FINGER_LEFT =    "\uD83D\uDC48"; //👈
    public static final String FINGER_RIGHT =   "\uD83D\uDC49"; //👉
    public static final String HANDS_OPEN =     "\uD83D\uDC50"; //👐
    public static final String MONEY_BAG =      "\uD83D\uDCB0"; //💰
    public static final String RED_SIREN =      "\uD83D\uDEA8"; //🚨
    public static final String PARTY_SMILEY =   "\uD83E\uDD73"; //🥳
}
